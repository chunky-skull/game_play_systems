...
✨The dialogue tree table would not keep track of which NPC gives quest or items. Instead that would be a special kind of NPC. Something like a join table that has an NPC's id, the quest's id, the dialogue node id, and potential item's id. The NPC's id may not be needed as the dialogue node id will give me access to which NPC this quest begins with.✨
